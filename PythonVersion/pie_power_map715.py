...
def pie_disp(wa, data_path, as_info_path, file_path_one, file_path_two, limit_count, limit_count_for_node):
    k = 2
    disp = False
    diff = False
...
def power_disp(wa, data_path, as_info_path, file_path, size_limit):
    k = 2
    disp = False
    diff = False
...
def map_disp(wa, data_path, as_info_path, file_path, size_limit):
    k = 2
    disp = False
    diff = False
    as_dict = read_as_location(as_info_path)
...
if __name__ == "__main__":
    ...
    node_limit_count = 1000  # 节点聚合阈值
    one_skip_pie_json, two_skip_pie_json = pie_disp(wa, data_path, info_path, pie_save_path_one, pie_save_path_two,
    ...
    size_limit = 2  # 边宽度约束值
    power_json = power_disp(wa, data_path, info_path, power_save_path, size_limit)
    print("引力图：")
    print(power_json)

    map_save_path = "/Users/mac/desktop/map.json"
    size_limit = 3  # 边宽度约束值
    map_json = map_disp(wa, data_path, info_path, map_save_path, size_limit)
    print("地图：")
    print(map_json)
