...
def get_k_skip_pie_data(skip_table, k, limit_count):
    if k == 0:
...
def pie_disp(wa, data_path, as_info_path, file_path_one, file_path_two, limit_count):
    k = 2
    disp = False
    diff = False
    skip_table = as_kskip(wa, data_path, as_info_path, k, disp, diff)

    # 一跳
    one_skip_result = get_k_skip_pie_data(skip_table, 1, limit_count)
    one_skip_result_json = json.dumps(one_skip_result)
    with open(file_path_one, 'w') as file_obj:
        json.dump(one_skip_result, file_obj)
    # 二跳
    two_skip_result = get_k_skip_pie_data(skip_table, 2, limit_count)
...
def power_disp(wa, data_path, as_info_path, file_path):
    k = 2
    disp = False
    diff = False
    as_dict = read_as_location(as_info_path)
    skip_table = as_kskip(wa, data_path, as_info_path, k, disp, diff)
...
def map_disp(wa, data_path, as_info_path, file_path):
    k = 2
    disp = False
    diff = False
    as_dict = read_as_location(as_info_path)
...
if __name__ == "__main__":
    wa = 54574
    info_path = "/Users/mac/desktop/as_country_with_geo.csv"
    data_path = "/Users/mac/desktop/snapshot.csv"

    pie_save_path_one = "/Users/mac/desktop/pie_one_skip.json"
    pie_save_path_two = "/Users/mac/desktop/pie_two_skip.json"
    limit_count = 10000
    one_skip_pie_json, two_skip_pie_json= pie_disp(wa, data_path, info_path, pie_save_path_one, pie_save_path_two, limit_count)
    print("饼状图：")
    print("一跳：")
    ...
    # power_json = power_disp(wa, data_path, info_path, power_save_path)
    # print("引力图：")
    # print(power_json)
    #
    # map_save_path = "/Users/mac/desktop/map.json"
    # map_json = map_disp(wa, data_path, info_path, map_save_path)
    # print("地图：")
    # print(map_json)
